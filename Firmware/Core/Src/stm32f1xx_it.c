...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    switch(slaveState)
    {
      ...
      case LIN_RECEIVING_ID:
				...
				{
					dbgIdButtons++;
					slaveState = LIN_SENDING_DATA_TO_MASTER_UART2; // wait for TxCpltCallback to confirm the transfer really finished
					HAL_UART_Transmit_IT(&huart2, buttonData, BUTTONS_LEN);
					needButtonsRefresh = 1; // actual (blocking) wheel poll happens in main loop
				}
				else if (rxByte == DIAG_RS_ID)
				{
					dbgIdDiag++;
					slaveState = LIN_RECEIVING_BREAK; // diag pass-through disabled in blocking design; count and skip
					HAL_IWDG_Refresh(&hiwdg);
				}
        ...
        if (rxByte == STATUS_ID)
				{
					...
					needBacklightRefresh = 1; // actual (blocking) send to wheel happens in main loop
					slaveState = LIN_RECEIVING_BREAK;
					HAL_IWDG_Refresh(&hiwdg);
				}
				if (rxByte == DIAG_ID)
