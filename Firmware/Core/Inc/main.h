...
extern void RefreshButtonsFromWheel(void);
extern void RefreshTempFromWheel(void);
extern void SendBacklightToWheel(uint8_t* data, uint8_t len);

extern volatile uint8_t needButtonsRefresh;
extern volatile uint8_t needTempRefresh;
extern volatile uint8_t needBacklightRefresh;

/* Live telemetry for in-car debugging via Watch window (SWD) */
extern volatile uint32_t dbgBreakCount;
extern volatile uint32_t dbgSyncOkCount;
extern volatile uint32_t dbgIdCount;
extern volatile uint8_t  dbgLastId;
extern volatile uint32_t dbgIdButtons;
extern volatile uint32_t dbgIdStatus;
extern volatile uint32_t dbgIdTemp;
extern volatile uint32_t dbgIdDiag;
extern volatile uint32_t dbgIdUnknown;
